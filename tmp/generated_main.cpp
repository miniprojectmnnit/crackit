...
class Solution { public: int bestTimetoBuyandSellStock(std::vector<int> prices){return prices.empty()?0:1;} };
...
int main() {
    std::vector<std::string> testInputs = { "[7,1,5,3,6,4]" };
    std::cout << "[";
    Solution sol;
    for (size_t i = 0; i < testInputs.size(); ++i) {
        ...
        if (args.size() != 1) {
          throw std::runtime_error("Expected 1 argument(s), got " + std::to_string(args.size()));
            }
              const std::vector<int> prices = parseVectorIntValue(args[0]);
        auto res = sol.bestTimetoBuyandSellStock(prices);
