...
function getLLM(options = {}) {
  ...
  if (keysToUse.length === 0) {
    log.error(
      "LLM",
      `❌ No keys available for model ${model}. Dynamic keys count: ${ctx.apiKeys?.length || 0}, Env key present: ${!!process.env.GROQ_API_KEY}`,
    );
    return new ChatGroq({
      model: model,
    ...
    log.info(
      "LLM",
      `🔗 Using ${keysToUse.length} key(s) for ${model} (${ctx.apiKeys && ctx.apiKeys.length > 0 ? "from USER WALLET" : "from SERVER ENV"})`,
    );
  }

  // Create an array of models, one for each key
  const models = keysToUse.map(
    (key) =>
      new ChatGroq({
        model: model,
        temperature: temperature,
  ...
  if (models.length > 1) {
    ...
    fallbackRunnable.withStructuredOutput = function (schema, config) {
      const primaryStructured = primaryModel.withStructuredOutput(
        schema,
        config,
      );
      const fallbackStructured = models
        .slice(1)
        .map((m) => m.withStructuredOutput(schema, config));
...
async function callGeminiWithFallback(prompt, options = {}) {
  log.warn(
    "LLM",
    "Using deprecated callGeminiWithFallback. Please migrate to getLLM.",
  );
